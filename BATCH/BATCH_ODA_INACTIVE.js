/*------------------------------------------------------------------------------------------------------/
| Program		: ODA Inactive batch.js
| Event			: BATCH Script
|
| Description	: runs nightly, Criteria: Get records with application status of Closed that do not have any child records or if child records do exist those records do not have any modifications in 
| 				  the past 180 days.  Action:  Update application status to Inactive and insert todays date into application status date
|
|
| Notes			: 
| Created by	: Suhail Wakil
| Created at	: 04/03/2018
/------------------------------------------------------------------------------------------------------*/
...
function mainProcess() {
    ...
    for (i in capIDList) {
        ...
        if (!childArray || childArray.length == 0)
        {
        	...
        	updateAppStatus("Inactive", "Update via Batch ODA Inactive batch", capId);
        }
