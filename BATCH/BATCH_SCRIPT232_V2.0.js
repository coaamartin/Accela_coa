/*Title : Passed MJ Inspection Automation Batch_SCRIPT232_V2.0
Purpose : Automatically schedule inspections and send out relevant notifications on quarterly cycle for MJ Licenses
Author: Erich von Trapp - ETech Consulting, LLC
Functional Area : Batch Job
*/
testMode = (aa.env.getValue("ScriptName") == "Test" ? true : false);
if (testMode) {
    var RECORD_TYPE = "Licenses/Marijuana/*/License";
    var EMAIL_TEMPLATE = "LIC MJ COMPLIANCE #232";
    var REPORT_TEMPLATE = "MJ_Compliance_Corrections_Letter";
    aa.env.setValue("CurrentUserID", "ADMIN");
    aa.env.setValue("RECORD_TYPE", RECORD_TYPE);
    aa.env.setValue("EMAIL_TEMPLATE", EMAIL_TEMPLATE);
    aa.env.setValue("REPORT_TEMPLATE", REPORT_TEMPLATE);
}
...
function getInspectorByInspID(iNumber){
    ...
    if (!inspUserObj) {
        logDebug("WARNING retrieving for inspection " + iNumber + " " + iObj.getInspectionType() + ", user: " + inspUserObj);
        return null;
    }
...
logDebug2("<br><font color=RED> Processing " + capIDList.length + " records</font><br>");
...
for (c in capIDList) {
    ...
    if (capStatus != "Active") {
        logDebug2("<font color=RED> Skipping record " + capIDString + " " + tmpCap.getCapType().getAlias() + "</font> "
            + "<font color=BLACK>Record status: " + capStatus + " must be 'Active'</font >");
        continue;
    }

    logDebug2("<font color=BLUE> Processing record " + capIDString + " " + tmpCap.getCapType().getAlias() + "</font> "
        + "<font color=BLACK>Record status: " + capStatus + "</font>");
...
function getCycleInspections(capId) {
    ...
    if (nextInspDate == null || nextInspDate == "") {
        logDebug2("<font color=RED> Skipping record; </font>" 
            + "<font color=BLACK>Next Inspection Date field is empty</font>");
        return false;
    } else {
    ...
    for (var i in capInspections) {
        var inspSchedDate = capInspections[i].getScheduledDate();
        if (inspSchedDate == null) {
            logDebug2("<font color=RED>Skipping inspection " + capInspections[i].getIdNumber() + " " + capInspections[i].getInspectionType() + "</font> "
                + "<font color=BLACK>Inspection Schedule Date is null, Status: " + capInspections[i].getInspectionStatus() + "</font>");
            continue;
        }
...
function scheduleNextInspections(cycleInspections) {
    ...
    for (i in cycleInspections) {
        if (cycleInspections[i].getInspectionStatus() == "Passed" || cycleInspections[i].getInspectionStatus() == "Passed - Minor Violations") {
            ...
            if (cycleInspections[i].getInspectionType().indexOf("MJ Building Inspections") != -1) {
                ...
                var inspIdNew = getScheduledInspId(inspType);
                copyGuideSheetItems(cycleInspections[i].getIdNumber(), inspIdNew);
    ...
    if (inspCounter != 0) {
        logDebug2("<font color=GREEN> Scheduled " + inspCounter + " new inspections</font>");
    } else {
...
function getInspectorByInspID(iNumber) {
    ...
    iObj = iObjResult.getOutput();
    logDebug("Retrieving inspection " + iNumber + " inspector: " + iObj.getInspector());
    inspUserObj = aa.person.getUser(iObj.getInspector().getFirstName(),iObj.getInspector().getMiddleName(),iObj.getInspector().getLastName()).getOutput();
    return (inspUserObj? inspUserObj.getUserID():null);
}
...
function copyGuideSheetItems(fromInspId, toInspId) {
    try {
        var itemCap = (arguments.length > 2 && arguments[2] ? arguments[2] : capId); //optional capId
        var gsGroups = (arguments.length > 3 && arguments[3] ? arguments[3] : null);
        var gsTypes = (arguments.length > 4 && arguments[4] ? arguments[4] : null);
        var gsStatuses = (arguments.length > 5 && arguments[5] ? arguments[5] : null);
        ...
        for (gsIdx in gsArr) {
            ...
            /*
            logDebug(".guideSheet[" + gsIdx + "]: " + gGuideSheetModel.guidesheetSeqNbr
                + ", Group: " + gGuideSheetModel.guideGroup
                + ", Type: " + gGuideSheetModel.guideType
                + (gGuideSheetModel.guideStatus ? ", Status: " + gGuideSheetModel.guideStatus : "")
                + (gGuideSheetModel.getIsRequired() ? ", Required" : "")
            ); */
            if (gsGroups && !exists(gGuideSheetModel.guideGroup, gsGroups)) continue;
            if (gsTypes && !exists(gGuideSheetModel.guideType, gsTypes)) continue;
            if (gsStatuses && !exists(gGuideSheetModel.guideStatus, gsStatuses)) continue;
            ...
            if (gGuideSheetItemModels) {
                for (var j = 0; j < gGuideSheetItemModels.size(); j++) {
                    ...
                    /*
                    logDebug("..guideSheetItem[" + j + "]: " + gGuideSheetItemModel.guideItemSeqNbr
                        + " " + gGuideSheetItemModel.guideItemText
                        + (gGuideSheetItemModel.guideItemStatus ? ", Status: " + gGuideSheetItemModel.guideItemStatus : "")
                        + (gGuideSheetItemModel.guideItemComment ? ", Comment: " + gGuideSheetItemModel.guideItemComment : "")
                        + (gGuideSheetItemModel.getIsRequired() ? ", Required" : "")
                    );
                    */
                }
            }
        ...
        if (guideSheetList.size() > 0) {
            ...
            if (copyResult.getSuccess()) {
                var guideSheets = guideSheetList.toArray();
                var guideSheetTypeList = []
                for (gsIdx in guideSheets) {
                    var gGuideSheetModel = guideSheets[gsIdx];
                    guideSheetTypeList.push(gGuideSheetModel.guideType);
                }
                logDebug("Successfully copied guideSheet items: " + guideSheetTypeList.join(","));
                return true;
            } else {
                logDebug("Failed to copy guideSheet items. Error: " + copyResult.getErrorMessage());
                return false;
            }
        }
    } catch (err) {
        showDebug = true;
        showMessage = true;
        var context = "copyGuideSheetItems";
