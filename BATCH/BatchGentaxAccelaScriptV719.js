					...
					for (x in parsedWCRElementWithChildren)
				{
					...
					if (x == "SiteID")
					{
						...
						if(test.getSuccess())
						{
							...
							{
								var capAddResult = aa.address.getAddressByCapId(newcapId).getOutput();
									aa.address.removeAddress(newcapId,Number(capAddResult[0].getAddressId()))
								...
								logDebug("Successfully updated " + parsedWCRElementWithChildren["SiteID"] + " from reference address");
							}
							else
							{	
						...
						else
						{
							...
							if(refad != "" && refad != null && parsedWCRElementWithChildren["GISAddressID"] != null && parsedWCRElementWithChildren["GISAddressID"] != "")
							{
								...
								aa.address.createAddressWithRefAddressModel(newcapId,refadmodel);
								logDebug("Successfully created cap address from reference address for " + parsedWCRElementWithChildren["SiteID"]);
							}
							else
							{					 
