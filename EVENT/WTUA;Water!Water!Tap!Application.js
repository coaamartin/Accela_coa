...
if(ifTracer(wfTask == "Fee Processing" && wfStatus == "Ready to Pay", 'wf:Fee Processing/Ready to Pay')){
    ...
    for(i in feeArray ){ var aFee=feeArray[i];  if(feeExists(aFee, "NEW")) invoiceFee(aFee, "FINAL"); }
    for(i in feeArray2){ var aFee=feeArray2[i]; if(feeExists(aFee, "NEW")) invoiceFee(aFee, "FINAL"); }
