...
/* End SDOT work-around to prevent payment notices on auto-approved (paid) ACA submissions */
	...
	//If supplied value is "PRIMARYLP" or null check and send to LP
	if (sendEmailToContactTypes == "PRIMARYLP" || sendEmailToContactTypes == null || sendEmailToContactTypes == '') {
		conTypeArray = conTypeArray.push("PRIMARYLP");
	}
	...
	for (z in conTypeArray) {
		...
		else if (conType == "PRIMARYLP") {
			vConObjArry = vConObjArry.push("PRIMARYLP")
		}
		else {
		...
		for (x in vConObjArry) {
			...
			{
				conEmail = getPrimLPEmailByCapId(capId).toUpperCase()
			}
