logDebug("***** Starting SEND_EMAIL_BLD_OTC *****");
...
try
{
    ...
    var recordCC = aa.env.getValue("emailCC");
	...
	rParams.put("toDate", toDate);
	var emailtemplate = "ODA Finalize Agenda #222";
	var report = generateReportFile("Pre Application Agenda_script", rParams, aa.getServiceProviderCode());
	//sendNotification("noreply@auroragov.org", emailTo, "", emailtemplate, tParams, [report]);
    sendNotification("noreply@auroragov.org", emailTo, recordCC, emailtemplate, tParams, [report]);
}
catch(e)
{
