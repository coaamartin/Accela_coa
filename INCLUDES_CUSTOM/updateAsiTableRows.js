...
function updateAsiTableRows(tableName, columnName, newValue, options) {
    ...
    if(asitTable) { //found table
        ...
        if(settings.rowIndex != null && asitTable[settings.rowIndex] != null) { //update specific row
            ...
            rtn = updAsiTableRow(tableName, asiTableRowIndexes[settings.rowIndex], settings)
        } else {    //update all rows
            for (var ea in asitTable) {
                row = asitTable[ea];
                val = row[columnName] ? row[columnName] : null;
                rtn = updAsiTableRow(tableName, asiTableRowIndexes[ea], settings)
            }
        }
        return true
    }
...
function updAsiTableRow(rowIndex) {
    ...
    **/
    ...
    {
        logDebug('setUpdateColumnValue(): rowIndex = ' + rowIndex + ', columnName = ' + columnName + ', columnValue = ' + columnValue);
